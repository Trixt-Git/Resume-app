...
- v1.11 (this branch only, diverges from v1.10 above) — swapped `MODEL` from `gpt-5-mini` to `gpt-4o-mini`, a same-provider model swap rather than a new provider seam. Because `gpt-4o-mini` supports an ordinary `temperature` argument (unlike the gpt-5 reasoning-model line), `temperature=0.2` came back and `reasoning_effort` was dropped entirely — otherwise identical to the OpenAI Chat Completions shape from v1.10 (`role: "system"` message, `max_completion_tokens`). This is a same-file, few-line diff; no architecture change.
...
| LLM backend | OpenAI API, model `gpt-4o-mini` (this branch; `gpt-5-mini` on `claude/openai-mini-fork-0uo92d`) | Cheap, strong instruction-follower; a same-provider model swap to compare against gpt-5-mini. Short recruiter chats cost fractions of a cent |
...
| Generation params | `max_completion_tokens=400`, `temperature=0.2` | `gpt-4o-mini` supports an ordinary temperature knob (unlike the gpt-5 reasoning line), so the original Haiku-era value carries over unchanged for factual consistency; recruiter-length answers |
...
| Model string | `gpt-4o-mini` (this branch; `gpt-5-mini` on `claude/openai-mini-fork-0uo92d`) |
